// *********************************************************************************************************************
//  RUTA: frontend/src/pages/DetalleReceta.js
// 
// 🔹 Este componente gestiona la vista detallada de una receta específica.
// 🔹 Integra el sistema de interacciones sociales (Yummys, Comentarios y Respuestas) y la gestión de favoritos.
// *********************************************************************************************************************

/**
 * @fileoverview Vista de detalle de receta.
 * Implementa la recuperación de datos mediante parámetros de ruta, lógica de interacción social animada
 * y un sistema de hilos de comentarios con moderación para el autor.
...
// Importación de activos visuales para la categorización de alérgenos
...
/**
 * Componente funcional DetalleReceta.
 * Centraliza la lógica de visualización, interacciones y recomendaciones laterales.
 * 
 * @returns {JSX.Element} Vista completa del detalle de la receta.
 */
function DetalleReceta() {
    ...
    // Estados de datos de la receta e interacciones
    ...
    // Estados de interfaz de usuario (Header y Menú)
    const [datosUsuarioHeader, setDatosUsuarioHeader] = useState(null);
    const [menuPerfilAbierto, setMenuPerfilAbierto] = useState(false);
    const perfilRef = useRef(null);

    // Diccionario de iconos para mapeo dinámico de categorías
    const mapaIconos = {
    ...
    /**
     * Efecto para la gestión de cierre de menús desplegables al detectar clics fuera del contenedor.
     */
    useEffect(() => {
    ...
    /**
     * Finaliza la sesión del usuario eliminando el rastro en LocalStorage y redirigiendo al Inicio.
     */
    const cerrarSesion = () => {
    ...
    /**
     * Carga asíncrona de los datos de la receta, recomendaciones laterales e información del perfil.
     */
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                // Recuperación de la receta principal
                ...
                // Carga de catálogo para la barra lateral (filtrando la receta actual)
                ...
                // Información complementaria del usuario logueado para el Header
                if (usuarioLogueado) {
        ...
        cargarDatos();
        window.scrollTo(0, 0); // Posicionamiento al inicio de página tras la navegación
    }, [id, idUsuarioActual]); 

    /**
     * Gestiona la reacción "Yummy" (Me gusta) con feedback visual animado.
     */
    const handleYummy = async () => {
        ...
        } catch (error) { console.error("Error al procesar Yummy"); }
    };

    /**
     * Publica un comentario de primer nivel en la receta.
     */
    const handlePublicarComentario = async () => {
        ...
        try {
            ...
            setNuevoComentario(""); 
        } catch (error) { console.error("Error al publicar comentario"); }
    };

    /**
     * Envía una respuesta anidada a un comentario existente.
     * 
    ...
    const handleEnviarRespuesta = async (comentarioId) => {
        ...
        try {
            ...
            setTextoRespuesta(""); 
        } catch (error) { console.error("Error al responder"); }
    };

    /**
     * Solicita confirmación y elimina un comentario de primer nivel.
     * 
    ...
    /**
     * Elimina una respuesta específica dentro de un hilo de comentarios.
     * 
     * @param {string} comentarioId - ID del comentario padre.
     * @param {string} respuestaId - ID de la respuesta hija.
     */
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    setComentarios(res.data);
                } catch (error) { console.error("Error al eliminar respuesta"); }
            }
        });
    };

    /**
     * Alterna el estado de la receta en la colección de favoritos del usuario.
     */
    ...
    // Validación de interacción previa del usuario
                    ...
                    <div className="receta-cuerpo-grid">
                        <div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', paddingBottom: '15px', borderBottom: '1px solid #eee' }}>
                                ...
                                {/* BLOQUE DEL AUTOR: Navegación al perfil del creador */}
                                <Link to={`/usuario/${receta.autor?._id || receta.autor}`} style={{ textDecoration: 'none', color: '#333', display: 'flex', alignItems: 'center', gap: '15px' }}>
                                    ...
                                    <div>
                                        <p style={{ fontSize: '18px', color: '#666', margin: '0 0 5px 0' }}>
                                            Receta de:{' '}
                                            <span style={{ color: '#D35400', fontWeight: 'bold' }}>{receta.nombreAutor}</span>
                                        </p>
                                                ...
                                                {com.id && (
                                                    <div className="acciones-comentario">
                                                        <button 
                                                            ...
                                                            onClick={() => {
                                                                if (!usuarioLogueado) {
                                                                    return Swal.fire('¡Ups!', 'Inicia sesión para conversar.', 'info');
                                                                }
                        ...
                        {/* CONTENIDOS TÉCNICOS DE LA RECETA */}
                        <div>
