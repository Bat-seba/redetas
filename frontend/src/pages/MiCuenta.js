// *********************************************************************************************************************
//  RUTA: frontend/src/pages/MiCuenta.js 
// 
// 🔹 Este componente representa el panel privado del usuario (Dashboard).
// 🔹 Permite la edición del perfil, gestión (CRUD) de las recetas publicadas y acceso a favoritos.
// *********************************************************************************************************************

/**
 * @fileoverview Vista del panel de control personal del usuario.
 * Gestiona la edición de metadatos de perfil (con soporte multimedia), el filtrado local 
 * y provee una interfaz tabular para la edición en línea y eliminación de recetas propias.
...
/**
 * Componente funcional MiCuenta.
 * 
 * @returns {JSX.Element|null} Interfaz del panel de control de usuario o null durante la redirección de seguridad.
 */
function MiCuenta() {
    const navigate = useNavigate();
    
    // Recuperación de la sesión actual almacenada en el cliente
    const usuarioLogueado = JSON.parse(localStorage.getItem('usuarioRedetas'));
    
    // Resolución de compatibilidad para el identificador del usuario (soporte para diferentes esquemas de base de datos)
    const idUsuario = usuarioLogueado ? (usuarioLogueado._id || usuarioLogueado.id) : null;
  
    // Estados principales de listas e interacciones de filtrado
    const [misRecetas, setMisRecetas] = useState([]); 
    const [textoBusquedaLocal, setTextoBusquedaLocal] = useState(''); 
  
    // Estados relativos a la gestión de la identidad y perfil del usuario
    const [usuarioData, setUsuarioData] = useState(null); 
    const [editandoPerfil, setEditandoPerfil] = useState(false);
    const [datosPerfilEdit, setDatosPerfilEdit] = useState({ bio: '', foto_perfil_url: '' });

    // Estados relativos a la edición en línea (Inline Editing) de las recetas en la tabla
    ...
    // Diccionario estático de categorías dietéticas
    const categoriasDisponibles = [
        "Sin huevo", "Sin leche", "Sin azúcar", 
        "Sin harinas", "Sin frutos secos", "Sin gluten"
    ];

    /**
     * Efecto de inicialización.
     * Asegura la restricción de acceso a usuarios no autenticados y obtiene los recursos asociados al perfil.
     */
    useEffect(() => {
        if (!usuarioLogueado) {
            navigate('/acceso');
        ...
        const cargarMisRecetas = async () => {
            try {
                ...
                // Filtrado en el cliente para extraer únicamente las recetas de la autoría del usuario actual
                ...
                console.error("Error cargando el catálogo personal de recetas", err); 
            }
        };
    ...
    /**
     * Alterna la interfaz al modo de edición de perfil, pre-poblando el estado temporal.
     */
    ...
    /**
     * Transmite los cambios de metadatos del perfil al servidor mediante multipart/form-data.
     */
    const guardarPerfil = async () => {
        try {
            ...
            // Determinación del tipo de carga: nuevo archivo binario o eliminación de imagen existente
            ...
            console.error("Error completo en actualización de perfil:", error);
    ...
    /**
     * Utilidad para generar dinámicamente la URL de previsualización (local o remota) de la imagen de perfil.
     * 
     * @param {File|string} foto - Archivo cargado temporalmente o ruta estática del servidor.
     * @returns {string|null} Cadena URL de origen.
     */
    ...
    /**
     * Elimina permanentemente una receta del repositorio tras validación explícita del usuario.
     * 
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire({ title: 'Error', text: 'No se pudo borrar.', icon: 'error', confirmButtonColor: '#D35400' }); 
                }
            } 
        });
    };
    ...
    /**
     * Activa el estado de edición en línea para un elemento de la tabla.
     * 
     * @param {Object} receta - Objeto con los metadatos de la receta a modificar.
     */
    ...
    /**
     * Sincroniza la manipulación de arreglos mediante entradas tipo checkbox.
     * 
    ...
    const manejarCheckboxEdicion = (e) => {
        ...
        else nuevas = nuevas.filter(c => c !== value);
        setDatosEdicion({ ...datosEdicion, categorias: nuevas });
    };

    /**
     * Valida y persiste los cambios estructurales de una receta en la base de datos.
     * 
     * @param {string} id - Identificador de la receta a sobrescribir.
     * @returns {Promise<boolean>} Estado de resolución de la operación.
     */
    const guardarCambios = async (id) => {
        ...
        try {
            ...
            // Inserción múltiple para arrays en FormData
    ...
    /**
     * Secuencia asíncrona: Aplica los cambios de la receta y la abre en nueva pestaña para su validación visual.
     * 
     * @param {string} id - Identificador de la receta manipulada.
     */
    ...
    /**
     * Utilidad para la renderización mixta de activos de imagen (Buffer local vs estático remoto).
     * 
     * @param {File|string} imagen - Entidad de archivo a analizar.
     * @returns {string} URI del recurso formateado.
     */
    ...
    // Procesamiento derivado para la búsqueda tabular reactiva
    const recetasFiltradas = misRecetas.filter(receta => receta.titulo.toLowerCase().includes(textoBusquedaLocal.toLowerCase()));

    // Bloqueo de renderizado condicional previo a la navegación de invitados
    ...
    return (
        <main className="main-content">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', backgroundColor: '#fdf8f5', padding: '20px', borderRadius: '15px' }}>
                ...
                {/* Contenedor del buscador local de publicaciones */}
                ...
                {/* Contenedor de acciones principales de navegación */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    
                    {/* Enlace a la vista de recetario de favoritos */}
                    <Link to="/mis-favoritos" className="boton-favorito" style={{ textDecoration: 'none' }}>
                        ⭐ FAVORITOS GUARDADOS
                    </Link>

                    {/* Enlace para la creación de nuevas publicaciones */}
